...
watch(expressionData, () => resolveExp())
function shouldHide(key) {
	return !(hideComponentData.value && hideComponentData.value.includes(key))
}
function resolveExp() {
	// 反解析 表达式
	if (expressionData.value) {
		const arr = expressionData.value.split(/\s+/)
		if (arr.length >= 6) {
			//6 位以上是合法表达式
			let obj = {
				second: arr[0],
...
onMounted(() => {
	expressionData.value = props.expression
	hideComponentData.value = props.hideComponent
})
</script>
	...
	<div>
		...
		<div class="popup-main">
			...
			<div class="pop_btn">
				...
				<ElButton @click="hidePopup">取消</ElButton>
			</div>
		</div>
	</div>
</template>
